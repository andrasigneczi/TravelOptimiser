...
#define NS_DECL_NSITCPSERVERSOCKETCHILD \
  NS_IMETHOD Listen(nsITCPServerSocketInternal *serverSocket, uint16_t port, uint16_t backlog, const nsAString & binaryType, JSContext* cx) override; \
  NS_IMETHOD Close(void) override; 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSITCPSERVERSOCKETCHILD(_to) \
  NS_IMETHOD Listen(nsITCPServerSocketInternal *serverSocket, uint16_t port, uint16_t backlog, const nsAString & binaryType, JSContext* cx) override { return _to Listen(serverSocket, port, backlog, binaryType, cx); } \
  NS_IMETHOD Close(void) override { return _to Close(); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSITCPSERVERSOCKETCHILD(_to) \
  NS_IMETHOD Listen(nsITCPServerSocketInternal *serverSocket, uint16_t port, uint16_t backlog, const nsAString & binaryType, JSContext* cx) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Listen(serverSocket, port, backlog, binaryType, cx); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsTCPServerSocketChild, nsITCPServerSocketChild)

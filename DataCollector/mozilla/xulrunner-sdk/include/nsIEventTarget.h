...
#define NS_DECL_NSIEVENTTARGET \
  NS_IMETHOD Dispatch(nsIRunnable *event, uint32_t flags) override; \
  NS_IMETHOD IsOnCurrentThread(bool *_retval) override; 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIEVENTTARGET(_to) \
  NS_IMETHOD Dispatch(nsIRunnable *event, uint32_t flags) override { return _to Dispatch(event, flags); } \
...
#define NS_FORWARD_SAFE_NSIEVENTTARGET(_to) \
  NS_IMETHOD Dispatch(nsIRunnable *event, uint32_t flags) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Dispatch(event, flags); } \

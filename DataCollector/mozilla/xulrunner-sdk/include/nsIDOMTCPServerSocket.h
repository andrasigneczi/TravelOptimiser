...
#define NS_DECL_NSIDOMTCPSERVERSOCKET \
  ...
  NS_IMETHOD GetOnconnect(JS::MutableHandleValue aOnconnect) override; \
  NS_IMETHOD SetOnconnect(JS::HandleValue aOnconnect) override; \
...
#define NS_FORWARD_NSIDOMTCPSERVERSOCKET(_to) \
  ...
  NS_IMETHOD GetOnconnect(JS::MutableHandleValue aOnconnect) override { return _to GetOnconnect(aOnconnect); } \
  NS_IMETHOD SetOnconnect(JS::HandleValue aOnconnect) override { return _to SetOnconnect(aOnconnect); } \
...
#define NS_FORWARD_SAFE_NSIDOMTCPSERVERSOCKET(_to) \
  ...
  NS_IMETHOD GetOnconnect(JS::MutableHandleValue aOnconnect) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetOnconnect(aOnconnect); } \
  NS_IMETHOD SetOnconnect(JS::HandleValue aOnconnect) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SetOnconnect(aOnconnect); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsDOMTCPServerSocket, nsIDOMTCPServerSocket)
...
#define NS_DECL_NSITCPSERVERSOCKETINTERNAL \
  NS_IMETHOD Init(JS::HandleValue windowVal) override; \
  NS_IMETHOD Listen(uint16_t localPort, JS::HandleValue options, uint16_t backlog) override; \
  NS_IMETHOD CallListenerAccept(nsITCPSocketChild *socketChild) override; \
  NS_IMETHOD CallListenerError(const nsAString & message, const nsAString & filename, uint32_t lineNumber, uint32_t columnNumber) override; 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSITCPSERVERSOCKETINTERNAL(_to) \
  NS_IMETHOD Init(JS::HandleValue windowVal) override { return _to Init(windowVal); } \
  NS_IMETHOD Listen(uint16_t localPort, JS::HandleValue options, uint16_t backlog) override { return _to Listen(localPort, options, backlog); } \
  NS_IMETHOD CallListenerAccept(nsITCPSocketChild *socketChild) override { return _to CallListenerAccept(socketChild); } \
  NS_IMETHOD CallListenerError(const nsAString & message, const nsAString & filename, uint32_t lineNumber, uint32_t columnNumber) override { return _to CallListenerError(message, filename, lineNumber, columnNumber); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSITCPSERVERSOCKETINTERNAL(_to) \
  NS_IMETHOD Init(JS::HandleValue windowVal) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Init(windowVal); } \
  NS_IMETHOD Listen(uint16_t localPort, JS::HandleValue options, uint16_t backlog) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Listen(localPort, options, backlog); } \
  NS_IMETHOD CallListenerAccept(nsITCPSocketChild *socketChild) override { return !_to ? NS_ERROR_NULL_POINTER : _to->CallListenerAccept(socketChild); } \
  NS_IMETHOD CallListenerError(const nsAString & message, const nsAString & filename, uint32_t lineNumber, uint32_t columnNumber) override { return !_to ? NS_ERROR_NULL_POINTER : _to->CallListenerError(message, filename, lineNumber, columnNumber); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsTCPServerSocketInternal, nsITCPServerSocketInternal)

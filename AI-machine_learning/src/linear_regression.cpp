...
arma::mat GradientDescent::calcL( const arma::mat& dataSet, const arma::mat& theta, double alpha, double lambda, long long iteration, long double threshold /*=0 */) {
    // cleaning of the Feature Scaling vector
    mFCData.clear();
...
arma::mat NormalEquation::calc( const arma::mat& dataSet, double lambda ) {
    // theta = (X.t()*X + lambda*L).i()*X.t()*y
    ...
    // let's insert a column filled with ones
    X.insert_cols(0, arma::ones<arma::mat>(m,1));
    arma::mat L = arma::eye( X.n_cols, X.n_cols );
    L(0,0) = 0.0;
    mTheta = (X.t()*X + lambda*L).i()*X.t()*Y;
    return mTheta;
}

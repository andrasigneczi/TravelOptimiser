...
class SOFTMAX_LAYER:
    ...
    def forward(self, X):
        ...
        dummy = np.exp(X - np.max(X, axis=0))
        self.Y = (dummy / dummy.sum(axis=1, keepdims=True))
    ...
    def backward(self, output):
        ...
        #return self.delta_X
        output[self.Y.argmax(axis=0),range(self.Y.shape[1])] -= 1
        return output/self.Y.shape[1]
